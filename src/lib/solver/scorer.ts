...
export function applyMultiplier(value: number, code: string): { val: number; wm: number } {
    const m = code.trim();
...
export function scoreMove(
    ...
    for (let i = 0; i < word.length; i++) {
        ...
        if (board[r][c] !== ' ') {
            ...
            if (hasNeighbor) {
                let tr = r;
                let tc = c;
                while (tr - dr2 >= 0 && tc - dc2 >= 0 && board[tr - dr2][tc - dc2] !== ' ') {
                    tr -= dr2;
                    tc -= dc2;
                }
                ...
                while (
                    ...
                    let v2 = cr2 === r && cc2 === c && val === 0 ? 0 : LETTER_VALUES[x] || 0;
                    if (cr2 === r && cc2 === c) {
                        const { val: axVal, wm: axm } = applyMultiplier(v2, BOARD_MULT[r][c]);
                        v2 = axVal;
                        cm *= axm;
                    }
                    cs += v2;
    ...
    if (br >= 0 && br < 15 && bc >= 0 && bc < 15 && board[br][bc] !== ' ') return null;
    const ar = d === 'V' ? sr + word.length : sr;
    const ac = d === 'H' ? sc + word.length : sc;
    if (ar >= 0 && ar < 15 && ac >= 0 && ac < 15 && board[ar][ac] !== ' ') return null;
