 ...
 */
export function generateBoard(numWords: number, dictionary: Set<string>, trie: Trie): string[][] {
    ...
    const board = emptyBoard();
    const word = [...dictionary].find(w => w.length >= 3 && w.length <= 5) || 'CAT';
...
export function boardToString(board: string[][]): string {
    const header = '    ' + Array.from({ length: 15 }, (_, i) => String(i).padStart(3)).join('');
 ...
 */
export function removeBoardTilesFromBag(board: string[][], bag: string[]): void {
    for (let r = 0; r < 15; r++) {
        for (let c = 0; c < 15; c++) {
            if (board[r][c] !== ' ') {
