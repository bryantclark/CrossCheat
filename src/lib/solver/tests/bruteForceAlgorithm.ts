...
function findAnchors(board: string[][]): { anchors: Set<string>; hasTiles: boolean } {
...
export function bruteForceSolve(
    ...
    if (numBlanks === 0) {
        candidates = [...dictionary].filter(w => [...new Set(w)].every(ch => available.has(ch)));
    } else if (numBlanks === 1) {
        ...
        for (const anchorKey of anchors) {
            ...
            for (const d of ['H', 'V'] as Direction[]) {
                for (let offset = 0; offset < word.length; offset++) {
                    ...
                    if (s !== null) {
                        resultsMap.set(key, { word, row: sr, col: sc, direction: d, score: s });
                    }
                }
            }
        }
