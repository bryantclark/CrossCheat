...
<main class="min-h-screen bg-slate-50 text-slate-900 font-sans p-4 lg:p-6 flex flex-col items-center" in:fade={{ duration: 600 }}>
	<header class="w-full max-w-7xl mb-6 flex items-center justify-between" in:fly={{ y: -20, duration: 800 }}>
		...
		<div class="flex gap-4">
			...
			{:else}
				<div class="px-4 py-2 bg-white rounded-full border border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2 shadow-sm">
					<div class="w-2 h-2 rounded-full {solving ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}"></div>
	...
	<div class="w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
		<div class="lg:col-span-8 flex flex-col gap-4" bind:clientHeight={leftColHeight} in:fade={{ delay: 200 }}>
			<div class="bg-white p-4 md:p-5 rounded-3xl shadow-xl border border-slate-100 relative group overflow-hidden">
				...
				<div class="flex items-center justify-between mb-2">
					...
					<button
						onclick={clearBoard}
						class="text-[10px] font-bold text-slate-400 hover:text-red-500 transition-colors uppercase tracking-widest border border-slate-200 px-2 py-1 rounded relative z-10"
					>
						Clear Board
					</button>
				</div>
				<div class="justify-center flex overflow-auto">
					<Board bind:board previewBoard={isPreviewing ? previewBoard : undefined} />
				</div>
				<div class="mt-3 flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-widest">
		...
		<div class="lg:col-span-4" style="height: {leftColHeight}px" in:fade={{ delay: 400 }}>
			<Results {results} onHover={handlePreview} onSelect={handleSelect} />
		</div>
	</div>

	<footer class="mt-12 text-slate-400 text-xs font-medium uppercase tracking-[0.2em] pb-8">
